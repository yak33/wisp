...
- 公网与代理出口异步补充国家、地区、城市和网络提供方，归属地失败不影响 IP 展示
- 百度、网易、阿里云、腾讯云、GitHub、Google、YouTube、Amazon 八站 HTTPS 响应耗时
- 所有网络项独立并发、独立失败；刷新代次会丢弃迟到的旧结果
...
| 直接打字 | 主页唤起即聚焦搜索框，打字过滤功能；剪贴板 / 备忘唤起时焦点在列表，点击搜索框后打字过滤条目 |
...
| `Ctrl+P` | 置顶 / 取消置顶（剪贴板） |
| `Delete` | 剪贴板多选（≥2）时清空选择；否则删除所选条目（剪贴板 / 备忘删除光标行） |
